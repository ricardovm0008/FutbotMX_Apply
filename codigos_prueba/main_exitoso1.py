...
def main():
    ...
    for frame in sv.get_video_frames_generator(VIDEO_PATH):
        # --- A. INFERENCIA CON TU MODELO YOLO ---
        resultados = model(frame, verbose=False)[0]
        detecciones = sv.Detections.from_ultralytics(resultados)
        
        # Dibujamos todas las cajas (Pelotas, Porterías y Robots) en el video original
        etiquetas = [
            f"{model.model.names[class_id]} {confidence:.2f}"
            for class_id, confidence in zip(detecciones.class_id, detecciones.confidence)
        ]
        frame = corner_annotator.annotate(scene=frame, detections=detecciones)
        frame = label_annotator.annotate(scene=frame, detections=detecciones, labels=etiquetas)
        
        # --- B. RADAR TÁCTICO 2D (Solo para la pelota, Clase 0) ---
        radar_canvas = crear_cancha_radar(RADAR_W, RADAR_H)
        
        # Filtramos para buscar solo la pelota (en tu archivo classes.txt, Pelota es la clase 0)
        pelota_detecciones = detecciones[detecciones.class_id == 0]
        
        if len(pelota_detecciones.xyxy) > 0:
            # Tomamos la caja de la primera pelota detectada
            x1, y1, x2, y2 = pelota_detecciones.xyxy[0] 
            
            punto_centro_x = (x1 + x2) / 2
            punto_centro_y = (y1 + y2) / 2
        ...
        # Dibujar estela naranja de la pelota
